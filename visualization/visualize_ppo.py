"""
PPO 训练结果可视化脚本
生成 PPO 训练的分析图表，包括奖励曲线、损失曲线、温度变化等
"""
import os
import pandas as pd
import numpy as np
...
# ==================== PPO 日志 ====================
LOG_DIR = 'log/ppo_pretest_6_4/ppo/Feb27-230953'
OUTPUT_DIR = 'log/ppo_pretest_6_4'
...
def plot_test_reward(ax, ea):
    ...
    test_epochs = list(range(len(test_rewards)))
    test_vals = np.array([r.value for r in test_rewards])

    # 过滤异常值
    valid_mask = (test_vals > -50000) & (test_vals < 10000)
    valid_epochs = np.array(test_epochs)[valid_mask]
    valid_vals = test_vals[valid_mask]

    ax.plot(valid_epochs, valid_vals, alpha=0.5, label='raw', linewidth=1)
    window = 20
    if len(valid_vals) > window:
        smoothed = np.convolve(valid_vals, np.ones(window)/window, mode='valid')
        ax.plot(range(window-1, len(valid_vals)), smoothed, 'purple', linewidth=2, label=f'ma({window})')
    ax.axhline(y=0, color='gray', linestyle='--')
    ax.set_xlabel('Epoch')
    ax.set_ylabel('Test Reward')
    ax.set_title('1. PPO: Test Reward Trend')
    ax.legend()
...
def plot_loss(ax, ea):
    """绘制损失曲线"""
    has_data = False
    for tag, color, max_thresh in [('update/loss/clip', 'blue', 10), ('update/loss/vf', 'red', 100000), ('update/loss/ent', 'green', 10)]:
        try:
            losses = ea.Scalars(tag)
            vals = np.array([l.value for l in losses])
            # 过滤异常值
            valid_mask = (vals >= 0) & (vals < max_thresh)
            valid_vals = vals[valid_mask]
            valid_epochs = np.arange(len(vals))[valid_mask]

            if len(valid_vals) > 0:
                has_data = True
                smoothed = np.convolve(valid_vals, np.ones(10)/10, mode='valid')
                ax.plot(range(9, len(valid_vals)), smoothed, label=tag.split('/')[-1], color=color, linewidth=1.5)
    ...
    ax.set_xlabel('Epoch')
    ax.set_ylabel('Loss')
    ax.set_title('3. PPO: Loss Curves (smoothed)')
    ax.legend()
    ax.grid(True, alpha=0.3)
...
def main():
    ...
    plt.tight_layout()
    output_path = os.path.join(OUTPUT_DIR, 'training_analysis_ppo.png')
    ...
    valid_vals = [v for v in test_vals if -50000 < v < 10000]

    if valid_vals:
        print(f"\n=== PPO 训练统计 ===")
        print(f"有效数据点: {len(valid_vals)}/{len(test_vals)}")
        print(f"最佳 reward: {max(valid_vals):.2f}")
        print(f"平均 reward: {np.mean(valid_vals):.2f}")
