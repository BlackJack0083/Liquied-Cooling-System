"""
SAC 训练结果可视化脚本
生成 SAC 训练的分析图表，包括奖励曲线、损失曲线、温度变化等
"""
import os
import pandas as pd
import numpy as np
...
# ==================== SAC 日志 ====================
LOG_DIR = 'log/sac_pretest_12_4/sac/Mar01-000838'
OUTPUT_DIR = 'log/sac_pretest_12_4/sac/Mar01-000838'
...
def plot_test_reward(ax, ea):
    ...
    ax.set_title('1. SAC: Test Reward (0-500 epoch)')
    ax.legend()
...
def plot_loss(ax, ea):
    """绘制损失曲线"""
    has_data = False
    # SAC TensorBoard 使用 update/loss/actor 格式
    ...
    ax.set_xlabel('Epoch')
    ax.set_ylabel('Loss')
    ax.set_title('3. SAC: Loss Curves (0-100 epoch)')
    ax.legend()
    ax.grid(True, alpha=0.3)
...
def main():
    ...
    plt.tight_layout()
    output_path = os.path.join(OUTPUT_DIR, 'training_analysis_sac_new.png')
